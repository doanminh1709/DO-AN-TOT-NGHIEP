...
export class EditProductComponent implements OnInit {
  ...
  constructor(private modalService: NgbModal, private categoryService: CategoryService, private productService: ProductService, private toastr: ToastrService, private uploadService: UploadService) {
    this.postForm = new FormGroup({
      ...
      'author': new FormControl(null, [Validators.minLength(4), Validators.required]),
      'pageSize': new FormControl(null, [Validators.min(50), Validators.required]),
      'suitableReadingAge': new FormControl(null, [Validators.min(4), Validators.required]),
      'language': new FormControl('Vietnamese'),
      'size': new FormControl(null , [Validators.min(4), Validators.required]),
      'publisher': new FormControl(null , [Validators.min(10), Validators.required]),
    })
  }
  ...
  getProduct() {
    this.productService.getOne(this.id).subscribe(data => {
      ...
      this.postForm = new FormGroup({
        ...
        'author': new FormControl(this.product.author ,[Validators.minLength(4), Validators.required]),
        'pageSize': new FormControl(this.product.quantity, [Validators.min(1), Validators.required]),
        'suitableReadingAge': new FormControl(this.product.suitableReadingAge, [Validators.min(1), Validators.required]),
        'language': new FormControl(this.product.language),
        'size': new FormControl(this.product.size, [Validators.minLength(4), Validators.required]),
        'publisher': new FormControl(this.product.publisher, [Validators.minLength(4), Validators.required]),
      })
