...
public interface ProductRepository extends JpaRepository<Product, Long>{
	...
	@Query(value = "select * from  products where category_id in ?1" , nativeQuery = true)
	List<Product> findByListCategoryId(List<Long> ids);
	...
	@Query(value = "(Select p.*, avg(r.rating) Rate From products p \r\n"
			...
			+ "Where p.category_id != ?\r\n"
			+ "group by p.product_id , p.name)\r\n"
			+ "Order by categoryS_id = ? desc, Rate desc", nativeQuery = true)
